...
pub const WAL_NAME: &str = "characters";
...
impl DataStruct {
    ...
    pub fn identity_opt(&self) -> Option<u32> {
        if self.identity == 0 { None } else { Some(self.identity) }
    }
    ...
    pub fn from_bytes(raw: &[u8]) -> Self {
        ...
        ds.identity   = u32::from_le_bytes(raw[0..4].try_into().unwrap());
        ds.created_at = u64::from_le_bytes(raw[4..12].try_into().unwrap());
        ds.updated_at = u64::from_le_bytes(raw[12..20].try_into().unwrap());
