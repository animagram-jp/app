...
pub fn open_modal() -> Vec<DomCmd> {
    ...
    cmds.push(DomCmd::new(Operation::SetText, "modal_fieldset-2_table_thead_th-4", None, Some("合計")));
    cmds.push(DomCmd::new(Operation::SetText, "modal_fieldset-2_table_thead_th-5", None, Some("")));
    ...
    cmds.push(DomCmd::new(Operation::SetText, "modal_fieldset-3_table_thead_th-5", None, Some("合計")));
    cmds.push(DomCmd::new(Operation::SetText, "modal_fieldset-3_table_thead_th-6", None, Some("")));
...
// Characteristic: input-1(初期値) + input-2(変動値) → span(合計) をリアルタイム更新
pub fn on_characteristic_input(row: usize, base: i32, delta: i32) -> Vec<DomCmd> {
    let total = (base + delta).max(1);
    let span_id = format!("modal_fieldset-2_table_tr-{}_span", row);
    vec![DomCmd::new(Operation::SetText, &span_id, None, Some(&total.to_string()))]
}

// Skill: 職業pt(input-1) か 興味pt(input-2) が変わったら合計spanを更新する
pub fn on_skill_input(row: usize, base: u16, occ_pt: u16, int_pt: u16) -> Vec<DomCmd> {
    let total = base + occ_pt + int_pt;
...
pub fn roll_characteristic(row: usize, char_vals: &mut [[i32; 2]; 9]) -> Vec<DomCmd> {
    if row == 0 || row > Characteristic::all().len() { return vec![]; }
    let val = Characteristic::all()[row - 1].generate() as i32;
    char_vals[row - 1][0] = val; // 初期値を上書き、変動値は据え置き
    let total = (val + char_vals[row - 1][1]).max(1);
...
pub fn roll_all_characteristics(char_vals: &mut [[i32; 2]; 9]) -> Vec<DomCmd> {
...
pub fn reset_modal(char_vals: &mut [[i32; 2]; 9], skill_pts: &mut [[u16; 2]; 20]) -> Vec<DomCmd> {
    *char_vals = [[0; 2]; 9];
    *skill_pts = [[0; 2]; 20];
